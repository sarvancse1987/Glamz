...
using Glamz.Domain.AdminSearch;
using Glamz.Domain.Blogs;
using Glamz.Domain.Catalog;
using Glamz.Domain.Cms;
...
using Glamz.Domain.PushNotifications;
    ...
    public partial class InstallationService : IInstallationService
    {
        protected virtual async Task InstallSettings(bool installSampleData)
        {
            try
            {
                ...
                {
                    DefaultAdminLanguageId = _languageRepository.Table.Count() > 0 ? _languageRepository.Table.Single(l => l.Name == "English").Id : "en_US",
                ...
                {
                    PrimaryStoreCurrencyId = _currencyRepository.Table.Count() > 0 ? _currencyRepository.Table.Single(c => c.CurrencyCode == "USD").Id : "USD",
                    PrimaryExchangeRateCurrencyId = _currencyRepository.Table.Count() > 0 ? _currencyRepository.Table.Single(c => c.CurrencyCode == "USD").Id : "USD",
                ...
                {
                    BaseDimensionId = _measureDimensionRepository.Table.Count() > 0 ? _measureDimensionRepository.Table.Single(m => m.SystemKeyword == "centimetres").Id : "centimetres",
                    BaseWeightId = _measureDimensionRepository.Table.Count() > 0 ? _measureWeightRepository.Table.Single(m => m.SystemKeyword == "lb").Id : "lb",
                });
                ...
                if (eaGeneral != null)
                {
                    await _settingService.SaveSetting(new EmailAccountSettings
                    {
                        DefaultEmailAccountId = "saravanan@rubixtek.com"//eaGeneral.Id
                    });
                }
